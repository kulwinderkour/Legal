...
import { SafetyRail } from "@/features/safety/ui/SafetyRail";
...
export function AskDocument({ clauses, provider, onNavigateToClause }: { clauses: Clause[], provider: AnalysisProvider, onNavigateToClause: (id: string) => void }) {
  ...
  const handleSubmit = (e: React.SyntheticEvent) => {
    ...
    provider.askQuestion(clauses, query).then(response => {
  ...
  return (
    <div className="h-full flex flex-col p-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="mb-12 relative">
        <input type="text" value={query} onChange={(e) => { setQuery(e.target.value); }} placeholder="Ask something about this document" className="w-full pl-6 pr-32 py-5 text-lg rounded-2xl border-2 border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 focus:border-blue-500 focus:ring-0 outline-none transition-all shadow-sm" />
        <button type="submit" disabled={!query.trim() || isSubmitting} className="absolute right-3 top-3 bottom-3 px-6 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 font-medium rounded-xl hover:bg-neutral-800 transition-colors disabled:opacity-50">
...
function Suggestions({ setQuery }: { setQuery: (q: string) => void }) {
  const suggestions = ["What is the monthly rent?", "What is the notice period?", "Should I sign this agreement?"];
  return (
